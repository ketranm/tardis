...
This implementation use stochastic attention aka hard attention.
This is a demo how to use hard attention.
Might need to merge code in one file and give more options.
--]]

require 'model.Transducer'
require 'model.HardAttention'
...
function NMT:backward(input, target)
    ...
    local reward = logProb:gather(2, target)
    -- all good. Ready to backprop

    local gradLoss = self.criterion:backward(logProb, target:view(-1))
    ...
    local gradGlimpse = self.glimpse:backward({outputEncoder, outputDecoder, reward}, gradLayer[1])
