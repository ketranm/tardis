...
local MemoryBlock, parent = torch.class('nn.MemoryBlock', 'nn.Module')

function MemoryBlock:__init(memorySize, memInp, memOut, posMat)
    parent.__init(self)
...
function MemoryBlock:__slice(mem)
    --[[ Slicing memory `mem`
...
function MemoryBlock:updateOutput(input)
...
function MemoryBlock:backward(input, gradOutput)
    local x, h = unpack(input)
    local N, T, D = h:size(1), h:size(2), h:size(3)
    local Mx = self.memorySize
...
function MemoryBlock:clearState()
    nn.utils.clear(self, {
        'output',
        'gradInput',
        'ht',
